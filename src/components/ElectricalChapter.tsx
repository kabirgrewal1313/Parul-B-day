...
const facts = [
  "Still investigating why she chose EE.",
    ...
    <section className="chapter overflow-hidden bg-[#f8fbff]">
      ...
      <div className="chapter-inner grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="reveal">
          ...
          <h2 className="font-display text-6xl leading-none text-ink sm:text-8xl">The Assignment Boss Fight</h2>
          <p className="mt-8 max-w-xl text-lg leading-8 text-ink/70">
            A lighthearted tech chapter with floating diagrams, electric particles, and the eternal
            question that academia refuses to answer.
          </p>
        </div>
        <div className="reveal relative min-h-[560px] overflow-hidden border border-white/80 bg-white/45 p-8 shadow-crystal backdrop-blur-2xl">
          ...
          <div className="relative grid h-full gap-5">
            {[Cpu, Zap, RadioTower].map((Icon, index) => (
              <div className="glass-panel flex items-center gap-5 p-6" key={facts[index]}>
                <div className="grid h-14 w-14 place-items-center rounded-full bg-white text-ink shadow-glow">
                  <Icon size={24} />
                </div>
                <p className="font-display text-3xl leading-tight">{facts[index]}</p>
              </div>
            ))}
